...
while read line; do 	
	...
	MA=$(bc <<< "scale=2; (${arr[1]})*4")
	PH=$(bc <<< "scale=2; (${arr[2]})*4")
	BE=$(bc <<< "scale=2; (${arr[3]})*3")
	BEE=$(bc <<< "scale=2; (${arr[4]})*3")
	BEEE=$(bc <<< "scale=2; (${arr[5]})*3")
	EE=$(bc <<< "scale=2; (${arr[6]})*3")
	ANS=$(bc <<< "scale=2; ($MA)+($PH)+($BE)+($BEE)+($BEEE)+($EE)+(${arr[7]})+(${arr[8]})+(${arr[9]})")
